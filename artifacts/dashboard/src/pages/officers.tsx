...
import { Users, Plus, Trash2, Trophy, Phone, MapPin, Briefcase, X, Eye, EyeOff } from "lucide-react";
    ...
    <Layout>
      <div className="p-4 md:p-6 space-y-6 max-w-6xl">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold tracking-tight text-slate-900">Kelola Petugas</h1>
            <p className="text-sm text-slate-500 mt-0.5">Manajemen akun dan performa tim lapangan</p>
          </div>
          <div className="flex gap-2">
            <button
              className="text-sm font-semibold text-slate-600 border border-slate-200 px-3 py-2 rounded-xl hover:bg-slate-50 transition flex items-center gap-1.5"
            >
              ⬇ Export Performa
            </button>
            <button
              onClick={() => { setShowForm(true); setError(""); }}
              className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded-xl text-sm transition"
            >
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white rounded-2xl border border-slate-100 p-4">
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Petugas</div>
            <div className="text-3xl font-extrabold text-slate-900" style={{ letterSpacing: "-0.04em" }}>{petugas.length}</div>
          </div>
          <div className="bg-white rounded-2xl border border-slate-100 p-4">
            ...
            <div className="text-3xl font-extrabold text-slate-900" style={{ letterSpacing: "-0.04em" }}>
              {petugas.reduce((s, o) => s + Number(o.totalInput), 0).toLocaleString()}
            </div>
          </div>
          <div className="bg-white rounded-2xl border border-slate-100 p-4">
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Event</div>
            <div className="text-3xl font-extrabold text-slate-900" style={{ letterSpacing: "-0.04em" }}>
              {Math.max(...petugas.map((o) => Number(o.totalEvent)), 0)}
            </div>
          </div>
        </div>
              ...
              {sorted.slice(0, 5).map((o, i) => (
                <div key={o.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-400">{o.jabatan} · {o.wilayah} · {o.totalEvent} event</div>
                  </div>
                  <div className="text-lg font-extrabold text-slate-800">{Number(o.totalInput).toLocaleString()}</div>
                </div>
              ))}
                ...
                {filtered.map((o) => (
                  <div key={o.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="text-right shrink-0">
                      <div className="text-sm font-bold text-slate-800">{Number(o.totalInput).toLocaleString()}</div>
