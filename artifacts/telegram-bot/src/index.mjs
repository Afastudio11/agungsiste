...
var groq = process.env.GROQ_API_KEY ? new Groq({ apiKey: process.env.GROQ_API_KEY }) : null;
...
var GROQ_PROMPT = `Kamu adalah OCR untuk KTP Indonesia. Ekstrak data dari foto KTP ini dan kembalikan JSON dengan field berikut (null jika tidak ditemukan):
{
  "nik": "16 digit NIK",
...
async function scanKtpWithGroq(imageBase64) {
...
async function getImageBase64(fileUrl) {
  ...
  const jpegBuf = await sharp(Buffer.from(buf)).resize({ width: 1200, withoutEnlargement: true }).jpeg({ quality: 90 }).toBuffer();
  return jpegBuf.toString("base64");
}
async function getActiveStaff() {
  return db.select({ id: usersTable.id, name: usersTable.name, username: usersTable.username }).from(usersTable).orderBy(usersTable.name);
}
async function getActiveEvents() {
  const now = /* @__PURE__ */ new Date();
  return db.select({ id: eventsTable.id, name: eventsTable.name, location: eventsTable.location }).from(eventsTable).where(
    and(
...
async function registerParticipant(data) {
  ...
  if (existing) {
    const totalEvents2 = await db.$count(
    ...
    return { alreadyRegistered: true, isNew, totalEvents: totalEvents2 };
  }
...
function buildPetugasKeyboard(staff) {
...
function buildEventKeyboard(events) {
  const kb = new InlineKeyboard();
  events.forEach((e) => {
...
var pendingKtp = /* @__PURE__ */ new Map();
...
bot.callbackQuery(/^event:(\d+):(.+)$/, async (ctx) => {
  ...
  const confirmKb = new InlineKeyboard().text("Daftarkan", "confirm:yes").text("Batal", "confirm:no");
  await ctx.editMessageText(
...
bot.on("message:document", async (ctx) => {
  ...
  if (state.step === "await_photo" || state.step === "idle") {
    await ctx.reply(
      "Untuk hasil OCR terbaik, kirim foto sebagai *foto* (bukan file/dokumen). Di Telegram, pilih gambar lalu pilih 'Send as Photo'.",
      { parse_mode: "Markdown" }
    );
  }
});
...
bot.start({ allowed_updates: ["message", "callback_query"] }).then(() => console.log("Bot stopped")).catch((err) => {
